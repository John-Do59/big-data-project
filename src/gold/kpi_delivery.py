...
def calculate_avg_delay(orders):
    """Calculate average delivery delay, only for DELIVERED orders with both dates."""
    logistics_base = orders.filter(
...
def calculate_late_rate(orders):
    """Calculate percentage of DELIVERED orders that arrived late."""
...
def calculate_delay_impact(orders, reviews):
    """Calculate impact of late delivery on customer reviews, only for DELIVERED orders."""
    logistics_base = orders.filter(
        (F.col("order_status") == "delivered") &
        F.col("order_delivered_customer_date").isNotNull() &
        F.col("order_estimated_delivery_date").isNotNull()
    ).join(reviews, "order_id", "left").withColumn(
        "delivery_delay_days",
        F.datediff("order_delivered_customer_date", "order_estimated_delivery_date")
    ).withColumn(
